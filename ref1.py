...
def get_data_osm():
    ...
    # Preparing the string for the Overpass request - Halle + Saalekreis + Berlin but it fails with HTTP 400
    # overpass_data_out = '?data=[out:json];area["de:amtlicher_gemeindeschluessel"="15002000"]->.halle;area["de:amtlicher_gemeindeschluessel"="11000000"]->.berlin;('
    # overpass_vegan_objects = 'node(area.halle)["diet:vegan"~"yes|only|limited"];way(area.halle)["diet:vegan"~"yes|only|limited"];node(area.berlin)["diet:vegan"~"yes|only|limited"];way(area.berlin)["diet:vegan"~"yes|only|limited"]'
    # overpass_vegetarian_objects = 'node(area.halle)["diet:vegetarian"~"yes|only"];way(area.halle)["diet:vegetarian"~"yes|only"];node(area.berlin)["diet:vegetarian"~"yes|only"];way(area.berlin)["diet:vegetarian"~"yes|only"]'
    # overpass_out = ');out+center;'
    
    overpass_data_out = '?data=[out:json];area["de:amtlicher_gemeindeschluessel"="11000000"]->.berlin;area["de:amtlicher_gemeindeschluessel"="15088"]->.saalekreis;('
    overpass_vegan_objects = 'node(area.berlin)["diet:vegan"~"yes|only|limited"];way(area.berlin)["diet:vegan"~"yes|only|limited"];node(area.saalekreis)["diet:vegan"~"yes|only|limited"];way(area.saalekreis)["diet:vegan"~"yes|only|limited"];'
    overpass_vegetarian_objects = 'node(area.berlin)["diet:vegetarian"~"yes|only"];way(area.berlin)["diet:vegetarian"~"yes|only"];node(area.saalekreis)["diet:vegetarian"~"yes|only"];way(area.saalekreis)["diet:vegetarian"~"yes|only"];'
    overpass_out = ');out+center;'
